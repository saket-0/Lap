...
// --- 3. Session Setup ---
// Ensure the session table exists before mounting the session store. connect-pg-simple expects the table to exist.
const ensureSessionTableAndSetup = async () => {
    try {
        // Create the table if it doesn't exist. Use a simple schema compatible with connect-pg-simple.
        await pool.query(`
        ...
        // Now mount the session middleware with the store
        app.use(session({
            store: new PgSession({
        ...
        console.error('Failed to ensure session table or mount session middleware:', e);
        process.exit(1);
    }
};

// Run the ensure/setup immediately and then register routes/listen after session middleware is mounted
ensureSessionTableAndSetup().then(() => {
    ...
    // Require authentication first so req.session.user exists.
    ...
    app.put('/api/users/:id/role', isAuthenticated, async (req, res) => {
        ...
        // Prevent admin from demoting themselves (compare as strings)
    ...
    console.error('Failed to initialize session table and start server:', err);
    process.exit(1);
});